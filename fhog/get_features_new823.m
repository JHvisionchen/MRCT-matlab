...
elseif n == 3
    ...
    xr = histcImWin(imrgb(:,:,1),nbins,ones(nwindow,nwindow),'same');
    xr=xr(cell_size:cell_size:end,cell_size:cell_size:end,:);
    xg = histcImWin(imrgb(:,:,2),nbins,ones(nwindow,nwindow),'same');
    xg = xg(cell_size:cell_size:end,cell_size:cell_size:end,:);
    xb = histcImWin(imrgb(:,:,3),nbins,ones(nwindow,nwindow),'same');
    xb = xb(cell_size:cell_size:end,cell_size:cell_size:end,:);
...
%    x=cat(3,x, h1, h2, ha, hb);%83.5
   x=cat(3,x, xr,xg,xb, h1, h2, ha, hb);%83.5
%    x = h2;
